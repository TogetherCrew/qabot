...
To achieve this goal effectively, you should consider generating subquestions that will help break down the main question into smaller, more manageable parts. These subquestions might involve understanding the context using the [TOOLS] available, the individuals involved, and the specific details related to goal question.
...
def get_template() -> PromptTemplate:
    ...
    PROMPT = PromptTemplate(
        input_variables=["name", "role", "goal", "subquestions_list"], template=template)
    return PROMPT
