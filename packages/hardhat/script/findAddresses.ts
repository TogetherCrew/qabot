...
async function main() {
  // path join
  const pathAdd = path.join(
    __dirname,
    "../broadcast/deploy.anvil.s.sol/31337/run-latest.json"
  );
  ...
  addresses.transactions.forEach((tx: any) => {
    if (tx.transactionType === "CREATE" && tx.contractName === "ERC1967Proxy") {
      if ("arguments" in tx && tx.arguments?.length > 0) {
        ...
        const key = Object.keys(objContracts).find(
          (key) => objContracts[key] === tx.arguments[0]
        );
        if (key) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(objContracts, null, 2)
  );
}
